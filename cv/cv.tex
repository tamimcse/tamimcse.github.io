...
\begin{document}
\name{MD I. Islam}
\tagline{PhD Candidate, Computer Science}
...
\cvtag{C}
\cvtag{C++}
\cvtag{Router Dataplane Algorithms}\\
...
\begin{itemize}
	\item Developed Trie (CP-Trie, Poptrie and SAIL) based routing table lookup algorithms in C and ASIC. 
	\item Evaluated the algorithms with routes from real core routers.
\end{itemize}
\smallskip
\cvproject{C2RTL: A High-level Synthesis tool}
\begin{itemize}
	\item Developed a high-level synthesis tool that can generate synthesizable Verilog RTL for pipelined ASIC from C code.
